...
func resolveStatsRange(req UsageStatsRequest) (from, to time.Time, err error) {
	...
	case "7", "14", "30", "90":
		...
		if err != nil || n <= 0 {
			n = statsDefaultRangeDays
		}
		from = to.AddDate(0, 0, -(n - 1))
		from = time.Date(from.Year(), from.Month(), from.Day(), 0, 0, 0, 0, now.Location())
	case "custom":
		f, ferr := time.ParseInLocation(statsDateLayout, req.From, now.Location())
		t, terr := time.ParseInLocation(statsDateLayout, req.To, now.Location())
		if ferr != nil || terr != nil {
			return time.Time{}, time.Time{}, errors.New("usage stats: custom range needs valid from/to dates (2006-01-02)")
		}
		if t.Before(f) {
			return time.Time{}, time.Time{}, errors.New("usage stats: custom range from date must not be after to date")
		}
		today := time.Date(now.Year(), now.Month(), now.Day(), 0, 0, 0, 0, now.Location())
		if t.After(today) {
			return time.Time{}, time.Time{}, errors.New("usage stats: custom range to date must not be in the future")
		}
	...
	default:
		...
		from = to.AddDate(0, 0, -(statsDefaultRangeDays - 1))
		from = time.Date(from.Year(), from.Month(), from.Day(), 0, 0, 0, 0, now.Location())
	}
	return from, to, nil
}
