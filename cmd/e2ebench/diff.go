...
func resetTree(repo string) {
	...
	_ = exec.Command("git", "-C", repo, "clean", "-fd", "-e", "hiq.toml", "-e", "hiq.toml").Run()
}
...
func renderDiff(r diffReport) string {
	...
	fmt.Fprintf(&b, "## 🤖 hiq e2e — diff test-gen\n\n")
...
// repoRelFromModulePath turns "github.com/zzycxz/hiq/internal/agent/foo.go" into
