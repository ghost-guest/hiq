...
func TestSaveImageDataURL(t *testing.T) {
	...
	if !strings.HasPrefix(got, ".hiq/attachments/clipboard-") || !strings.HasSuffix(got, ".png") {
...
func TestSaveAttachmentFile(t *testing.T) {
	...
	if !strings.HasPrefix(got, ".hiq/attachments/clipboard-") || !strings.HasSuffix(got, ".pdf") {
...
func TestImageDataURLRejectsOutsideAttachmentDir(t *testing.T) {
	...
	if _, err := ImageDataURL("../.hiq/attachments/x.png"); err == nil {
...
func TestImageDataURLRejectsSymlinkFile(t *testing.T) {
	...
	link := filepath.Join(".hiq", "attachments", "link.png")
...
func TestImageDataURLRejectsSymlinkAttachmentDir(t *testing.T) {
	t.Chdir(t.TempDir())
	if err := os.Mkdir(".hiq", 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.Mkdir("elsewhere", 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.Symlink("../elsewhere", filepath.Join(".hiq", "attachments")); err != nil {
		t.Skipf("symlink unsupported: %v", err)
	}
	if _, err := ImageDataURL(".hiq/attachments/x.png"); err == nil {
...
func TestImageDataURLRejectsSymlinkSubdirectory(t *testing.T) {
	...
	link := filepath.Join(".hiq", "attachments", "link")
