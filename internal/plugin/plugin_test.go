...
func TestStartAvailableKeepsGoodServers(t *testing.T) {
	...
	bad := Spec{Name: "bad", Command: "hiq-missing-mcp-binary"}
...
func TestStdioCommandNotFoundSuggestsPATHFix(t *testing.T) {
	...
	host, _ := StartAvailable(ctx, []Spec{{Name: "missing", Command: "hiq-missing-mcp-binary"}})
	defer host.Close()
	...
	for _, want := range []string{
		`command "hiq-missing-mcp-binary" not found on PATH`,
