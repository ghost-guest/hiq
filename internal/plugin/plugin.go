// Package plugin is hiq's MCP client. It connects to external MCP servers and
...
// protocolVersion is the MCP revision hiq advertises during initialize.
...
type Spec struct {
	...
	// inherit hiq's cwd (the default for user-configured plugins). It exists
...
// messages (notifications, requests like roots/list) are ignored — hiq is a
...
func (c *Client) initialize(ctx context.Context) error {
	res, err := c.call(ctx, "initialize", map[string]any{
		...
		"clientInfo":      map[string]any{"name": "hiq", "version": "dev"},
	})
	if err != nil {
		return err
	}
...
type mcpTool struct {
	...
	// plugin that declares a tool read-only opts it into hiq's parallel-dispatch
