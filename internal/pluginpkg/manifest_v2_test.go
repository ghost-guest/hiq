...
func TestMigrateProvidersRequiresExplicitProvides(t *testing.T) {
	pkg := Package{ManifestKind: "fairpeer", Manifest: Manifest{
...
func TestMigrateRejectsVersionedManifest(t *testing.T) {
  ...
  "name": "already-v2"
	...
	if _, _, err := ParseNativeForMigrate(root); err == nil || !strings.Contains(err.Error(), "already uses fairpeer.io/plugin/v2") {
