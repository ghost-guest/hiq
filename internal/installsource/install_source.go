...
func (t *installSourceTool) Execute(ctx context.Context, raw json.RawMessage) (string, error) {
	...
	if len(actions) == 0 {
		out := response{
			...
			PlanID:   planID,
			Warnings: warnings,
			Next:     "No installable hiq skill or MCP server was detected. Ask the user for a direct SKILL.md, skill root, .mcp.json, MCP endpoint, or package name.",
		}
...
func (t *installSourceTool) resolveSkillPath(name, scope string) (string, bool) {
	...
	if scope == "global" {
		...
		root = filepath.Join(t.home, ".hiq", skill.SkillsDirname)
	} else {
		root = filepath.Join(t.root, ".hiq", skill.SkillsDirname)
	}
...
func (t *installSourceTool) configPath(scope string) string {
	...
	return filepath.Join(t.root, "hiq.toml")
}
