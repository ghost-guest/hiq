...
func (t *installSkillTool) Description() string {
	scope := "'global' (only option — no project workspace) writes to ~/.hiq/skills/."
	if t.store.HasProjectScope() {
		scope = "'project' (default) writes to <repo>/.hiq/skills/ (this workspace only); 'global' writes to ~/.hiq/skills/ (every project)."
	}
...
"type":"object",
"properties":{
  "name":{"type":"string","description":"Identifier — letters/digits/_/-/., 1-64 chars, starts alphanumeric. Becomes the skill folder name under ~/.hiq/skills/<name>/SKILL.md."},
