...
func New(cfg Config) provider.Provider {
	...
	if built, err := netclient.NewHTTPClient(cfg.Proxy, netclient.TransportOptions{
		...
		TLSHandshakeTimeout: 15 * time.Second, ResponseHeaderTimeout: provider.ResponseHeaderTimeout(),
	}); err == nil {
		httpClient = built
	}
	...
	return &client{
		identityHeaders: newClientIdentityHeaders(),
		name:            cfg.Name,
...
func (c *client) send(ctx context.Context, body map[string]any) (*http.Response, error) {
	...
	newRequest := func(ctx context.Context) (*http.Request, error) {
		...
		applyOpenCodeGoHeaders(req, c.baseURL, c.identityHeaders)
...
func (c *client) readStream(ctx context.Context, resp *http.Response, out chan<- provider.Chunk, requestMessages []provider.Message, maxOutputTokens int) {
	...
	for scanner.Scan() {
		...
		case "response.function_call_arguments.delta":
			...
			if !sendChunk(ctx, out, provider.Chunk{Type: provider.ChunkToolArgsDelta, ToolCall: &provider.ToolCall{ID: call.id, Name: call.name}, Text: event.Delta}) {
				return
			}
