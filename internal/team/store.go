...
// SetContext replaces the team's leader-editable blackboard and bumps its
// version.
//
// Notes and Checkpoints are NOT part of that editing surface — one is written
// by members while they work, the other is generated by the checkpoint pass —
// so they are carried forward from the existing context when the caller leaves
// them out. Without this, saving the blackboard from the panel silently wiped
// the team's entire shared scratchpad as a side effect of editing the goal.
func (s *Store) SetContext(teamID string, ctx TeamContext) (Team, error) {
	return s.Update(teamID, func(t *Team) {
		if len(ctx.Notes) == 0 {
			ctx.Notes = t.Context.Notes
		}
		if len(ctx.Checkpoints) == 0 {
			ctx.Checkpoints = t.Context.Checkpoints
		}
...
func assignMissingIDs(t *Team) {
	...
	for i := range t.Context.Checkpoints {
		if t.Context.Checkpoints[i].ID == "" {
			t.Context.Checkpoints[i].ID = newID("ckpt")
		}
		if t.Context.Checkpoints[i].At.IsZero() {
			t.Context.Checkpoints[i].At = time.Now().UTC()
		}
	}
	for i := range t.Tasks {
		if t.Tasks[i].ID == "" {
