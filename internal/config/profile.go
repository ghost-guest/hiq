	...
	return []Profile{
		{
			Name:        ProfileDev,
			DisplayName: "编码",
			// Skill whitelist: dev mode is coding-domain. The remaining office
			// skills (desktop/ppt/email/rag/schedule/document/expert), the ops
			// reference card (netdev-help) and browser-auto are all disabled —
			// they don't appear in the index and run_skill reports them disabled.
			// (browser-auto joined this list on 2026-09-20 when the built-in
			// browser was taken offline: the whitelist must not advertise a
			// capability whose tools no longer exist.)
			// Users who want the rest back can override in hiq.toml:
			//   [[profiles]]
			//   name = "dev"
