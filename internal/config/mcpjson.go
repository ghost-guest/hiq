...
// mcpJSONFile is the project-root file Claude Code calls .mcp.json. hiq reads
...
// legacyConfigPath is the v0.x (TypeScript line) config file, ~/.hiq/config.json.
func legacyConfigPath() string {
	home, err := os.UserHomeDir()
	if err != nil {
		return ""
	}
	return filepath.Join(home, ".hiq", "config.json")
}

// loadLegacyMCP reads the v0.x ~/.hiq/config.json and returns its enabled
...
// already declare. hiq.toml's [[plugins]] win on a name collision: it is the
// hiq-specific, more explicit of the two, so it overrides the shared,
